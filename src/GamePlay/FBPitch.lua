...
local FBPitch = class("FBPitch", function()
    local obj = FBPitch.new()
    return obj
end)
...
function FBPitch:init(ww, hh)
        ...
        self.m_penaltyArea[MatchDefs.SIDE.LEFT] = cc.rect(0,175,150,300);
        self.m_penaltyArea[MatchDefs.SIDE.RIGHT] = cc.rect(850, 175, 150, 300);
        self.m_Goals[MatchDefs.SIDE.LEFT] = cc.p(0, 170)
        self.m_Goals[MatchDefs.SIDE.RIGHT] = cc.p(1000, 170)
        ...
        local gpw = m_width / m_gridWidth;
        local gph = m_height / m_gridHeight;
        ...
        for  y = 1, gh do
            for x = 1, gw do
                ...
                local grid = require("GamePlay.FBPitchGrid")
                ...
                if (x < midW) then
                    local side = MatchDefs.SIDE.LEFT;
...
--                        matchDefs:computeGridShootAngleAndMat(goalGate, &grid);
                ...
                elseif (x > midW) then
                    local side = matchDefs.SIDE.RIGHT;
                    local goalGate = self.m_Goals[side];
                    goalGate.y = self.m_height * 0.5;
...
function FBPitch:getBestAssistantDeffendingPosition(targetPos, side)
    ...
    return cc.pAdd(targetPos, cc.pMul(gp, matchDefs.ASSIST_DEFEND_DIST))
end
...
--            if ( not matchDefs.isPlayersOnTheWayToGoal(players, grid)) then
...
function FBPitch:isOffside(pos, side)
    ...
    elseif (side == matchDefs.SIDE.RIGHT) then
        if (pos.x > offsidePos) then
            return true;
        end
    end
...
function FBPitch:getOtherSide(side)
    if (side == matchDefs.SIDE.LEFT) then
        return matchDefs.SIDE.RIGHT
    elseif (side == matchDefs.SIDE.RIGHT) then
        return matchDefs.SIDE.LEFT
    end

    return matchDefs.SIDE.NONE;
end
